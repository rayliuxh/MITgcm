...
state2_file = 'BACKUP_succ_noseamount_4000m/state.0000000000.t001.nc';
...
% contourf(x,y,eta(:,:,it)','linestyle','none')
hold on
quiver(x,y,up(:,:,11,it)'*vscale,vp(:,:,11,it)'*vscale,0,'k')
title(['Temperature at t=',num2str(it)])
colorbar
pause
end
...
% for it = 1:30:size(eta,3)
it = 1000;
contourf(x,y,dspd_p(:,:,ik,it))
hold on
quiver(x,y,dup(:,:,ik,it)'*vscale,dvp(:,:,ik,it)'*vscale,0,'k')
