...
t3d_CH = [28.   , 27.992, 27.984, 27.976, 27.968, 27.96 , 27.952, 27.944, ...
       27.936, 27.928, 27.884, 27.804, 27.724, 27.644, 27.564, 27.484, ...
       27.404, 27.324];
   
   t3d_deep2 = t3d_deep;
   for iy = 1:size(t3d_deep,1)
       t3d_deep2(iy,:) = t3d_CH;
   end
...
% fid=fopen('OB_WestH.bin','w',ieee); fwrite(fid,eta,accuracy); fclose(fid);
 fid=fopen('OBzonalU_velo4.bin','w',ieee); fwrite(fid,u3d,accuracy); fclose(fid);
 fid=fopen('OBzonalT_velo4.bin','w',ieee); fwrite(fid,t3d_deep2,accuracy); fclose(fid);
